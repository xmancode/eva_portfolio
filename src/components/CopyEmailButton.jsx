...
function CopyEmailButton() {
  ...
  return (
    <motion.button onClick={copiedClipBoard} 
        whileHover={{y:-5}}
        whileTap={{scale:1.05}}
    className="relative px-1 py-4 text-sm text-center
     rounded-full font-extralight bg-primary w-[12rem] cursor-pointer overflow-hidden">
        ...
        {copied? (
            <motion.p className="flex justify-center items-center gap-2"
            key="copied"
        ...
        >
            <img src='assets/copy.svg' className='w-5' alt="copy icon"/>Copy email Address</motion.p>)}
            </AnimatePresence>
        </motion.button>
